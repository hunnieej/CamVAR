...
from utils.lr_control import filter_params_var_tune

import dist
from utils import arg_util, misc
from dataset.data import build_dataset,build_dataset_webtar
...
def build_everything(args: arg_util.Args):
    ...
    text_encoder,in_dim_cross=build_text(pretrained_path=args.text_enc_path,device=dist.get_device())
    ...
    vae_local, var_wo_ddp = build_vae_var(
        ...
        rope_norm=args.rope_norm, rope_theta=args.rope_theta,
    ...
    if args.from_scratch:
        ...
        print("load pre-trained var in %s"%args.var_ckpt)
        var_wo_ddp.load_state_dict(torch.load(args.var_ckpt, map_location='cpu'), strict=False)
    ...
    names, paras, para_groups = filter_params_var_tune(var_wo_ddp)
    opt_clz = partial(torch.optim.AdamW, betas=(0.9, 0.95), fused=args.afuse)
...
# def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
        ...
        is_val_and_also_saving = True #(ep + 1) % 10 == 0 or (ep + 1) == args.ep#每10 epoch验证一次
        if is_val_and_also_saving:
            if dist.is_local_master():
                ...
                trainer.inference_pic(args,text_encoder,cur_ep=ep,cur_iter=-1,top_k=4096,top_p=0.95,w_mask=True)
    ...
    for it, obj in me.log_every(start_it=start_it, 
        ...
        if it % args.val_it == 0:
            if dist.is_local_master():
                ...
                trainer.inference_pic(args,text_enc,cur_ep=ep,cur_iter=it,top_k=4096,top_p=0.95,w_mask=True)
                print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt}', flush=True, clean=True)
        ...
        grad_norm, scale_log2 = trainer.train_sampler_masked(
