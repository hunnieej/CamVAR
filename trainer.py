...
class VARTrainer(object):
    ...
    def eval_ep(self, args, ld_val, text_enc):
        ...
        for _obj_idx, obj in enumerate(ld_val):
            ...
            x_BLCv_wo_first_l: Ten = self.quantize_local.idxBl_to_var_input(
                gt_idx_Bl
            )  # 返回的是scale的插值结果
            ...
            if not drop_idxs == None:
                ...
                L_tail += (
                    self.val_loss(
                        logits_BLV.data[:, last_l:].reshape(-1, V),
                        gt_BL[:, last_l:].reshape(-1),
                    )
                    * B
                )
                ...
                acc_tail += (
                    logits_BLV.data[:, last_l:].argmax(dim=-1) == gt_BL[:, last_l:]
                ).float().mean() * 100
    ...
    def train_step(
        ...
        gt_idx_Bl: List[ITen] = self.vae_local.img_to_idxBl(
            inp_B3HW
        )  # gt_idx_Bl:所有scale对应的离散token的真值

        gt_BL = torch.cat(gt_idx_Bl, dim=1)
        x_BLCv_wo_first_l: Ten = self.quantize_local.idxBl_to_var_input(
            gt_idx_Bl
        )  # 返回的是scale的插值结果，注意x_BLCv_wo_first_l没有第一个scale
    ...
    def train_sampler_masked(
        ...
        gt_idx_Bl = self.vae_local.img_to_quant_embed(
            inp_B3HW
        )  # gt_idx_Bl:所有scale对应的离散token的真值
        gt_BL = torch.cat(gt_idx_Bl, dim=1)
        # self.from_idx=9
        self.from_idx = self.var_wo_ddp.from_idx
        bg, _ = self.begin_ends[self.from_idx]
        _, ed = self.begin_ends[-1]
        gt_BL = gt_BL[:, bg:ed]
        ...
        with self.var_opt.amp_ctx:
            ...
            logits_sampler, mask = self.var.module.forward_sampler(
                x_BLCv_wo_first_l=x_BLCv_wo_first_l,
                encoder_hidden_states=encoder_hidden_states,
                encoder_attention_mask=attn_mask,
                encoder_pool_feat=pooled_embed,
            ...
            logits_sampler_masked = torch.cat(
                [
                    logits_sampler * (1 - mask[..., None]),  # B,L,V
                    (torch.ones_like(logits_sampler[..., -1]) * 10 * mask).unsqueeze(
                        -1
                    ),
                ],  # B,L,1
                dim=-1,
            ...
            loss = self.train_loss(
                logits_sampler_masked.view(-1, V + 1), gt_BL_masked.view(-1)
            ).view(B, -1)
        ...
        if it == 0 or it in metric_lg.log_iters:
            Lmean = self.val_loss(
                logits_sampler_masked.data.view(-1, V + 1), gt_BL_masked.view(-1)
            ).item()
            acc_mean = (
                ((pred_BL * (1 - mask)) == gt_BL_masked).float().sum()
                / ((1 - mask).sum())
            ).item() * 100  # 只算预测的，不算mask的
            if prog_si >= 0:  # in progressive training
                Ltail = acc_tail = -1
            else:  # not in progressive training
                Ltail = Lmean
            ...
            if dist.is_master():
                ...
                for si, (bg, ed) in enumerate(self.begin_ends):
                    if 0 <= prog_si < si:
                        break
                    pred, tar = (
                        logits_sampler_masked.data[:, bg:ed].reshape(-1, V + 1),
                        gt_BL_masked[:, bg:ed].reshape(-1),
                    )
                    acc = (pred.argmax(dim=-1) == tar).float().mean().item() * 100
                    ce = self.val_loss(pred, tar).item()
                    kw[f"acc_{self.resos[si]}"] = acc
