...
__all__ = ["FFN", "AdaLNSelfAttn", "AdaLNBeforeHead", "AttnBlock", "ModifiedAttnBlock"]
...
def save_cross_attn(attn_map, save_name):  # [H,L,77]
    ...
    for idx in range(10):
        ...
        for scale, begin_end in enumerate(begin_ends):
            ...
            sns.heatmap(
                attn_cur.cpu().detach().numpy(), cmap="viridis", cbar=True, ax=ax
            )
...
def save_attn_map(attn, layer_num, attn_type="cross", attn_mask=None):
    ...
    attn_min = attn_softmax.min(dim=-1, keepdim=True)[
        0
    ]  # Get the minimum value along the -1 dimension
    attn_max = attn_softmax.max(dim=-1, keepdim=True)[
        0
    ]  # Get the maximum value along the -1 dimension
    ...
    if attn_type == "cross":
        ...
        sq_sum_attn_map = (
            torch.sqrt(torch.mean(attn**2, dim=0))
            .add_(attn_mask[0, 0])
            .cpu()
            .detach()
            .numpy()
...
class Attention(nn.Module):
    def __init__(
        ...
        self.block_idx, self.num_heads, self.head_dim, self.embed_dim = (
            block_idx,
            num_heads,
            embed_dim // num_heads,
            embed_dim,
    ...
    def forward(
        ...
        if not self.is_cross:
            ...
            if using_flash or self.using_xform:
                ...
                dim_cat = 1  # q or k or v: BLHc, freqs_cis: BLc//2
                if self.rotary_pos_emb:
                ...
                dim_cat = 2  # (b,l,3,num_h,h_dim)->(3,b,num_h,l,h_dim)->(b,h,l,c)
                if self.rotary_pos_emb:
                    q, k = apply_rotary_emb(q, k, freqs_cis=freqs_cis)
            # q or k or v: BHLc
        else:
            assert encoder_hidden_states is not None, (
                "hidden states of text prompts needed to be input as 'encoder_hidden_states'"
            )
            q = (
                F.linear(
                    input=x,
                    weight=self.mat_q.weight,  # 对哦，这不同scale的大小都不一样，cross-attention能一样吗
                    bias=self.q_bias,
                )
        ...
        if using_flash and (not self.is_cross):
            ...
            if not self.is_cross:
                ...
                oup = memory_efficient_attention(
                    ...
                    k,
                    v,
                    attn_bias=attn_bias.expand(B, self.num_heads, q.shape[1], -1),
                    p=dropout_p,
                    scale=self.scale,
                ).view(B, L, C)
        else:
...
class AdaLNSelfAttn(nn.Module):
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(
        self, x, cond_BD, attn_bias, encoder_hidden_states, encoder_attention_mask
...
class AttnBlock(nn.Module):
    ...
    def forward(
        ...
        x = x + self.drop_path(
            self.attn(
                self.ln_wo_grad(x).mul(scale1 + 1).add_(shift1),
                attn_bias=attn_bias,
                freqs_cis=freqs_cis,
                layer_id=layer_id,
            ).mul_(gamma1)
...
class ModifiedAttnBlock(AttnBlock):
    """
    Extended AttnBlock with ray adaptation capabilities.

    Adds RayAdapter as a parallel path to the frozen self-attention.
    CRITICAL: Ray adapter residual is added at the self-attention residual junction,
    BEFORE frozen cross-attention and FFN.
    """
    ...
    def forward(
        ...
        freqs_cis=None,
        layer_id=0,
        # Ray adaptation inputs (optional)
        memory=None,
        cam_dir=None,
        theta=None,
        ...
        # 2. Frozen self-attention
        dx_self_base = self.attn(
            u.mul(scale1 + 1).add_(shift1),
            attn_bias=attn_bias,
            freqs_cis=freqs_cis,
            layer_id=layer_id,
        )

        # 3. Ray adaptation (parallel path, only when enabled)
        if (
            self.enable_ray_adaptation
            and cam_dir is not None
            and memory is not None
            and theta is not None
        ):
            assert camera_embedder is not None, (
                "Camera embedder required for ray adaptation"
            )
        ...
        # 5. Frozen cross-attention (unchanged)
        if encoder_hidden_states is not None and self.enable_cross:
            if self.cross_attn_ln:
                x = (
                    self.cross_attn(
                        self.ln_wo_grad(x),
