    ...
    def f_to_idxBl_or_fhat(
        ...
        for si, (ph, pw) in enumerate(patch_hws):  # from small to large
            ...
            if self.using_znorm:  # False
                ...
                )  # (B*h*w, vocab_size)#(b*h*w,c),(c,vocab_size)
                idx_N = torch.argmin(d_no_grad, dim=1)
...
class MultiscaleVectorQuantizer2(nn.Module):
    ...
    # ===================== `forward` is only used in VAE training =====================
    def forward(
        self, f_BChw: torch.Tensor, ret_usages=True
    ) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        dtype = f_BChw.dtype
        if dtype != torch.float32:
            f_BChw = f_BChw.float()
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums):  # from small to large
                ...
                if self.using_znorm:  # 对codebook做l2 norm
                    rest_NC = (
                        F.interpolate(
                            f_rest,
                            size=(pn, pn),
                            mode="area" if si <= 3 else "bilinear",
                        )
                        .permute(0, 2, 3, 1)
                        .reshape(-1, C)
                        if (si != SN - 1)
                ...
                mean_commit_loss += F.mse_loss(
                    ...
                    F.interpolate(
                        f_BChw, size=(pn, pn), mode="area" if si <= 3 else "bilinear"
                    ),
        ...
        return (
            f_hat,
            (vq_loss, mean_commit_loss, mean_entropy_loss, usage),
            usages,
        )  # 先只调codebook
    ...
    def forward_new(
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            for si, pn in enumerate(self.v_patch_nums):  # from small to large
                ...
                mean_vq_loss += F.mse_loss(h_BChw, rest_NC) * (
                    pn**2
                )  # 这里的loss似乎有问题，后面的scale的loss可能受前面scale的影响
                # 如果能把每个scale的loss分开弄，训练应该会稳定一点？
                mean_commit_loss += F.mse_loss(f_hat.data, f_BChw).mul_(self.beta) * (
                    pn**2
                )
                mean_entropy_loss += (
                    self.entropy_loss_ratio * 0
                )  # compute_entropy_loss(-d)

            # 每个scale的loss平均
            mean_vq_loss *= 1.0 / sum([pn**2 for pn in self.v_patch_nums])
            mean_commit_loss *= 1.0 / sum([pn**2 for pn in self.v_patch_nums])
    ...
    def f_to_idxBl_or_fhat(
        ...
        for si, (ph, pw) in enumerate(patch_hws):  # from small to large
            ...
            f_hat_or_idx_Bl.append(
                f_hat.clone()
            )  # if to_fhat else idx_N.reshape(B, ph*pw))
    ...
    def get_next_autoregressive_input(
        ...
        if si != SN - 1:
            h = self.quant_resi[si / (SN - 1)](
                F.interpolate(h_BChw, size=(HW, HW), mode="bilinear"),
                len(self.v_patch_nums),
            )  # conv after upsample
            f_hat.add_(h)
            return f_hat, F.interpolate(
                f_hat,
...
class Phi_ada(nn.Conv2d):
    ...
    def forward(self, h_BChw, reso_cond):
        res_feat = (
            torch.tensor(reso_cond, dtype=torch.float32)
            .view(1, 1, -1)
            .to(h_BChw.device)
        )  # 1,C
        ...
        conv_result = nn.functional.conv2d(
            h_BChw, conv_weight, bias=self.bias, padding=self.padding
        )
