...
def compute_axial_cis(
    dim: int, end_x: int, end_y: int, theta: float = 100.0, normalize=32
):
    freqs_x = 1.0 / (
        theta ** (torch.arange(0, dim, 4)[: (dim // 4)].float() / dim)
    )  # 从1到1/theta的逐渐衰减的长为dim//4的序列
...
if __name__ == "__main__":
    ...
    for pn in pn_list:
        ...
        q = torch.ones([1, num_heads, pn**2, 64]).to(dtype)
        k = torch.ones([1, num_heads, pn**2, 64]).to(dtype)
        ...
        attn1_oup = (
            attn1[0, 0, 0].reshape(pn, pn).float()
        ).numpy()  # .astype(np.uint8)#BHLc
    ...
    def plot_value_list(value_list):
        ...
        for i, values in enumerate(value_list):
            x_max = len(values)
            axes[i].plot(
                [idx + 1 for idx in range(x_max - 1)],
                values[1:],
                linewidth=5,
                color="darkblue",
                alpha=0.3,
