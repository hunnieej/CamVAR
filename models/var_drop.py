...
class VAR(nn.Module):
    ...
    def from_logit2emb(self, logits_BlV, t, rng, top_k, top_p, B):  # 单个token的sample
        logits_BlV = (1 + t) * logits_BlV[:B] - t * logits_BlV[
            B:
        ]  # (B,l,4096)是每个token和vqvae codebook的4096个token的相似程度
        ...
        idx_Bl, conf = sample_with_top_k_top_p_(
            ...
            num_samples=1,
            return_conf=True,
        )  # 和哪个token最相似，对应的离散token的idx
        h_BChw = self.vae_quant_proxy[0].embedding(
            idx_Bl
        )  # B, l, Cvae=32，是再把确定的离散token的idx获得一个embed
        return h_BChw, conf
    ...
    def autoregressive_infer_cfg(
        ...
        if not self.noise_sampling:
            sos = self.encoder_proj(
                encoder_pool_feat
            )  # self.class_emb(torch.cat((label_B, torch.full_like(label_B, fill_value=self.num_classes)), dim=0))# (2*B,1024)
        else:
        ...
        if not self.rotary_pos_emb:
            lvl_pos = (
                self.pos_1LC
            )  # lvl_pos是level的pos emb和每层每个token的emb的和（所有的pos emb)
        ...
        if self.shared_aln:
            ...
            cond_lvl_emb = self.lvl_embed_adaln(
                cond_lvl_emb
            )  # cond_BD是加各种pos emb之前的class_emb
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            if w_mask and si >= self.from_idx:
                ...
                if sample_version == "1024":
                    step_thresh = 5
                    si_thresh = 13  # step<5；scale index<13的，直接用var输出的tokens
                ...
                for step in range(self.mask_scheduler.step):
                    ...
                    if step < step_thresh and si < si_thresh:
                        ...
                        logits_BlV = self.sample(
                            feature=cur_feature,  # x:2B,1,embed；当前token的feature，来自var
                            prev_emb=torch.cat([text_pool_feat, x], dim=1),
                            attn_bias=None,
                            freqs_cis=torch.cat(
                                [self.freqs_cis[0, None], freqs_cis_cur], dim=0
                            ),
                        )  # 2B,1,32；上一个token的emb，来自自回归模型
                    ...
                    # conf_Bl = torch.rand_like(conf_Bl)#根据conf_bl产生mask的，所以conf_bl随机就是mask随机
                    conf_Bl = (
                        torch.rand_like(conf_Bl) if step < 5 else conf_Bl
                    )  # 根据conf_bl产生mask的，所以conf_bl随机就是mask随机
            ...
            if si != self.num_stages_minus_1:  # prepare for next stage
                ...
                next_token_map = next_token_map.repeat(
                    2, 1, 1
                )  # double the batch sizes due to CFG (B,?,C)
    ...
    def forward(
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            sos = cond_BD = self.encoder_proj(
                encoder_pool_feat
            )  # (B,1024)#self.class_emb(label_B)
    ...
    def forward_sampler(
        ...
        return logits_BLV_[
            :, 1:
        ], mask  # logits BLV, V is vocab_size,V最后一个dim是masked token对应的embed
