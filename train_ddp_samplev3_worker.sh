...
/opt/miniconda3/bin/python -m torch.distributed.launch --nproc_per_node=$NPROC_PER_NODE --nnodes=$NNODES --node_rank=$node_rank --master_addr=$hostname --master_port=$PORT train_sampler.py \
