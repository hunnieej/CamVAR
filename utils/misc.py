...
def init_distributed_mode(local_out_path, only_sync_master=False, timeout=30):
    ...
    if (
        ...
        sys.stdout, sys.stderr = (
            SyncPrint(local_out_path, sync_stdout=True),
            SyncPrint(local_out_path, sync_stdout=False),
        )
...
class SyncPrint(object):
    ...
    def isatty(self):
        ...
        return (
            self.terminal_stream.isatty()
            if hasattr(self.terminal_stream, "isatty")
            else False
        )
                ...
                if len(self.meters) > 0:
                    for name, meter in self.meters.items():
                        if len(meter.deque) > 0:
                            if name in ["Lm", "Lt"]:
                                postfix_dict[name] = f"{meter.median:.3f}"
                            elif name in ["Accm", "Acct"]:
